...
export default function WorkerVerify() {
  ...
  const profile = (() => {
    try { return JSON.parse(localStorage.getItem('draftWorkerProfile') || '{}'); }
    catch { return {}; }
  })();
  ...
  const handleVerify = () => {
    ...
    setVerifying(true);
    setTimeout(() => {
      // Promote draft to final profile — worker AuthContext will read this
      localStorage.setItem('gigWorkerProfile', JSON.stringify(profile));
      localStorage.removeItem('draftWorkerProfile');
      // Login via the platform-level session context
      login('worker');
      setVerifying(false);
      setSuccess(true);
      setTimeout(() => navigate('/worker/dashboard', { replace: true }), 1800);
    }, 1200);
  };
      ...
      <main className="rs-shell" style={{ alignItems: 'center' }}>
        ...
        <div className="rs-form" style={{ textAlign: 'center' }}>
          ...
          <div style={{
            ...
            border: '1px dashed rgba(196,98,60,0.35)',
            borderRadius: '10px',
            padding: '10px 14px',
            marginBottom: '24px',
            fontSize: '0.82rem',
            ...
            🔒 <strong>Demo Mode</strong> — No real OTP is sent. Use code: <strong style={{ color: 'var(--rs-terracotta)', letterSpacing: '0.1em' }}>123456</strong>
          </div>
            ...
            {otp.map((digit, idx) => (
              <input
                ...
                style={{
                  ...
                  fontWeight: '700',
                  borderRadius: '12px',
                  border: `2px solid ${error ? 'var(--rs-terracotta)' : digit ? 'var(--rs-terracotta)' : 'rgba(42,33,89,0.2)'}`,
                  background: digit ? 'rgba(196,98,60,0.07)' : 'rgba(255,255,255,0.7)',
                  outline: 'none',
                  color: 'var(--rs-ink)',
          ...
          <button
            ...
            style={{ marginBottom: '16px', background: 'linear-gradient(120deg, var(--rs-terracotta), var(--rs-maroon))' }}
          >
          ...
          <div style={{ fontSize: '0.86rem', color: 'var(--rs-muted)', marginBottom: '16px' }}>
            {canResend ? (
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--rs-terracotta)', fontWeight: '600', display: 'inline-flex', alignItems: 'center', gap: '5px' }}
              >
          ...
          <Link to="/worker/register" style={{ fontSize: '0.84rem', color: 'var(--rs-terracotta)', textDecoration: 'none', fontWeight: '600' }}>
      ...
      {success && (
        <div className="rs-success-overlay">
          <div className="rs-success-modal">
            <div className="rs-success-icon" style={{ background: 'rgba(196,98,60,0.12)', color: 'var(--rs-terracotta)' }}>
              <ShieldCheck size={32} />
            </div>
            <h3 className="rs-success-title">Registration Successful!</h3>
            <p className="rs-success-text">Welcome to Kaarya! Redirecting to your worker dashboard…</p>
          </div>
        </div>
      )}
    </div>
